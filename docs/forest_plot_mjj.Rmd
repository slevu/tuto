---
title: "Faire un forest plot pour MJJ"
date: "12/04/23"
output:
  html_document:
    code_folding: hide
...
```{r read}
knitr::read_chunk("R/plot_effect_size3.R")
```
...
```{r show table, fig.width = 4, fig.height = 2}
( t1 <- make_table(d, x =  c('var', 'events', 'rr_ci'), lbl = c('Group', 'Events', 'RR (95%CI)'), h = 0.5) )
```

Et je combine les deux dans une même figure.

```{r join, warning = FALSE}
plot1 <- cowplot::plot_grid(t1, p0,
                    nrow = 1,
                   align = "h", rel_widths = c(.4, .2))
```

```{r show plot1, fig.width = 6, fig.height = 2}
plot1
```

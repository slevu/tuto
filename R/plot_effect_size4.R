...
#' groups group labels
## debug ng1 = 2; ng2 = 2; ng3 = 3; m = 10; n = 1e3; k = 2; sep = ", "; seed = 123
make_data <- function(groupsize = c(Study = 6), k = 2, sep = ", ", seed = 123){
  m <- prod(groupsize)
  x <- expand.grid( sapply(groupsize, seq, simplify = FALSE) )
  set.seed(seed)
  log_es <- rnorm(m)
  log_sd <- runif(m)*log_es
  n <- round( 1e3 / (exp(log_sd))^2 ) ## let's pretend
  event <- round(1e-2 * n * exp(log_es))
  bounds <- c(es = 0, lo = -1.96, up = 1.96)
  y <- round( exp( log_es + outer(abs(log_sd), bounds) ), k)
  data.frame(x, event, n, y)
}
# ( d <- make_data(groupsize = c(Study = 6, Sex = 2), sep = " to ", seed = 123) )
# prep_tab(d)
# 
# ci <- paste0("(", round(lo, k), sep, round(up, k), ")")
# es_ci <- paste(round(es, k), ci )
# data.frame(var, events, es, bounds , es_ci )
...
make_plot <- function(dt, x, lo, up,
                      ...
                      psize = NULL,
                      lw = 0.5,
                      limx = NULL ## vector of 2 bounds
                      ){
  dt <- prep_tab(dt)
  if (is.null(psize)){
    psize <- "psize"
    dt[, psize] <- GeomPoint$default_aes$size ## 1.5
  } 
  if (is.null(co)){
    co <- "cols"
    dt[, co] <- GeomPoint$default_aes$colour ## 1.5
  } 
  pos <- position_dodge2(width = dodgew)
  p0 <- ggplot(dt, aes(.data[[x]], .data[[y]], 
                       size = .data[[psize]], 
                       col = .data[[co]] ),
               show.legend = FALSE
               )
  pi <- if (is.null(limx)){
    p0 +
      geom_segment(aes(x = .data[[lo]], xend = .data[[up]], yend = .data[[y]]),
                   position = pos,
                   show.legend = FALSE,
                   linewidth = lw
                   # , col = co
                   ) ## error bars
  } else { ## make arrows if x limits
    if (any(dt[, x] < limx[1] | dt[, x] > limx[2])){
      stop("Effect size outside of CI limits, correct limx")
    } 
    p0 +
      geom_segment(data = dt[dt[, lo] > limx[1],],
                   aes(x = .data[[lo]], xend = .data[[x]], yend = .data[[y]]),
                   position = pos, show.legend = FALSE,
                   linewidth = lw) + ## left
      geom_segment(data = dt[dt[, up] < limx[2],],
                   aes(x = .data[[x]], xend = .data[[up]], yend = .data[[y]]),
                   position = pos, show.legend = FALSE,
                   linewidth = lw) + ## right
      geom_segment(data = dt[dt[, lo] <= limx[1], ],
                   aes(x = limx[1], xend = .data[[up]], yend = .data[[y]]),
                   position = pos, show.legend = FALSE,
                   arrow = arrow(length = unit(.15, "cm"), ends = "first"),
                   linewidth = lw ) + ## left arrow
      geom_segment(data = dt[dt[, up] >= limx[2],],
                   aes(x = .data[[x]], xend = limx[2], yend = .data[[y]]),
                   position = pos, show.legend = FALSE,
                   arrow = arrow(length = unit(.15, "cm"), ends = "last"),
                   linewidth = lw ) ## right arrow
  } ## arrows, cols could be added but need subsetting accordingly
  p1 <- pi +
  ...
  if (is.null(id2)) return(p1) else {
    p1 +
      facet_grid(rows = id2,
                 scales = "free",
                 # switch = "y", # unnecessary if no strip.text
                 space = "free") +
...
make_table <- function(dt, x, lbl,
  ...
  if (is.null(co)){
    co <- "cols"
    dt[, co] <- GeomPoint$default_aes$colour ## "black" or whatever unique value
  } 
  # dt[, id] <- factor(dt[, id], rev(unique(dt[, id]))) ## reverse order
  long_d <- reshape2::melt(dt[, c(id, id2, x, co)], id.vars = c(id, id2, co) )
  d0 <- ggplot(long_d, aes(x = .data[["variable"]], y = .data[[id]],
                                  label = .data[["value"]],
                           col = .data[[co]])) +
    geom_text(hjust = h, ...) +
